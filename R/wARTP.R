...
#'
#' @export
wARTP <- function(p_values, cor_mat = NULL, weight = NULL,
                  B1 = 5000, B2 = 50000, threshold = 0.05) {
  ...
  if (!is.null(weight)) {
    if (length(weight) != d || !is.numeric(weight) || any(weight < 0)) {
  ...
  if (!is.numeric(B1) || B1 < 1) stop("B1 must be a positive integer")
  if (!is.numeric(B2) || B2 < 1) stop("B2 must be a positive integer")
  if (!is.numeric(threshold) || threshold <= 0 || threshold >= 1) {
    stop("threshold must be between 0 and 1")
  }
  
  res1 <- wARTP_cpp(p_values, cor_mat, weight, as.integer(B1))
  p1 <- res1$p_value
  ss <- res1$signal_sparsity
  
  if (p1 <= threshold) {
    res2 <- wARTP_cpp(p_values, cor_mat, weight, as.integer(B2))
    return(list(p_value = res2$p_value,
                signal_sparsity = res2$signal_sparsity))
  }
  
  return(list(p_value = p1,
              signal_sparsity = ss))
}
